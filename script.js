...
let pdfBooks = JSON.parse(localStorage.getItem('pdfBooks')) || [];
...
// Add PDF book form event listener

const addPdfBookForm = document.getElementById('add-pdf-book-form');

if (addPdfBookForm) addPdfBookForm.addEventListener('submit', addPdfBook);
...
if (closeBorrowModal) closeBorrowModal.addEventListener('click', () => borrowModal.style.display = 'none');

if (borrowForm) borrowForm.addEventListener('submit', handleBorrow);
...
function handleLogin(e) {
    ...
    } else if (userType === 'teacher' && username === 'guru' && password === 'guru123') {

        currentUser = { type: 'teacher', username: 'guru' };
...
function showAdminFeatures() {
    ...
    document.getElementById('panel-title').textContent = 'Panel Admin';
...
function showTeacherFeatures() {
    ...
    document.getElementById('teacher-add-book').style.display = 'block';

    document.getElementById('add-book-form').style.display = 'none';

    document.getElementById('manage-students').style.display = 'none';

    document.getElementById('panel-title').textContent = 'Panel Guru';
...
function updateUI() {
    ...
    if (currentUser) {
        ...
        // Update logout button text based on user type
        ...
        if (currentUser.type === 'admin') {
            ...
            logoutBtn.textContent = 'Logout (Guru)';

            showTeacherFeatures();

        } else {

            logoutBtn.textContent = 'Logout (Siswa)';
...
function renderBooks() {
    ...
    books.forEach((book, index) => {
        ...
        if (currentUser && currentUser.type === 'student') {
            ...
            buttons += `<button class="borrow-btn" onclick="openBorrowModal('${book.title}')">Pinjam Buku</button>`;
    ...
    // Render PDF books for teachers and admins

    if (currentUser && (currentUser.type === 'teacher' || currentUser.type === 'admin')) {

        pdfBooks.forEach((book, index) => {

            const bookCard = document.createElement('div');

            bookCard.className = 'book-card pdf-book';

            bookCard.innerHTML = `

                <div class="book-info">

                    <h3 class="book-title">${book.title} (PDF)</h3>

                    <p class="book-synopsis">${book.synopsis}</p>

                    <button class="read-btn" onclick="readPdfBook(${index})">Baca PDF</button>
...
function addPdfBook(e) {

    e.preventDefault();

    const title = document.getElementById('pdf-book-title').value;

    const synopsis = document.getElementById('pdf-book-synopsis').value;
    ...
    const pdfBook = { title, synopsis, fileName: file.name };

    pdfBooks.push(pdfBook);

    localStorage.setItem('pdfBooks', JSON.stringify(pdfBooks));



    renderBooks();

    addPdfBookForm.reset();

    alert('Buku PDF berhasil ditambahkan!');

}



function readPdfBook(index) {

    const book = pdfBooks[index];

    // In a real app, this would open the PDF in a viewer

    // For now, we'll simulate opening it

    alert(`Membuka PDF: ${book.title}`);
...
function handleBorrow(e) {
    ...
    const bookTitle = document.getElementById('borrow-book-title').textContent;
